...
(function() {
    function _parse_attributes_and_content (attributes) {
        var content = Array.prototype.slice.call(arguments, 1);
        return typeof attributes == "object" ? [attributes].concat(content) :
                                               [[], attributes].concat(content);
    }

    Fern.render_tag_and_metadata = function (atom, params) {
        return typeof atom == "function" ? atom.apply(atom, params) : [atom];
    };

    Fern.render_tag = function (atom, params) {
        return Fern.render_tag_and_metadata(atom, params)[0];
    };

    function _stringify_key_value_pair(key, value, params) {
        return key + '="' + Fern.render_tag(value, params) + '"';
    }

    function _stringify_attribute_hash(attribute_hash, params) {
        var attributes = Object.keys(attribute_hash);
        ...
        return ' ' + attributes
                     .map(
                         function (attribute) {
                             return _stringify_key_value_pair(attribute, attribute_hash[attribute], params);
                         }
                     )
                     .join(' ');
    }

    function _range (lower, upper, step) {
        step = step || 1;
    ...
    function _stringify_attribute_array(attribute_array, params) {
        if (!attribute_array.length) {
            return '';
        }

        return ' ' + _range(0, attribute_array.length / 2 - 1)
                     .map(function (i) {
                         return _stringify_key_value_pair(attribute_array[2 * i],
                                                          attribute_array[2 * i + 1],
                                                          params);
                     })
                     .join(' ');
    }

    function _stringify_attributes(attributes, params) {
    ...
    function _stringify_content(content, params) {
        if (content === undefined) {
            return [''];
        }

        var all_content = content.map(function (thing) { return Fern.render_tag_and_metadata(thing, params); });
        var result = [all_content.map(function (thing) { return thing[0]; }).join('')];

        // Now append metadata
        all_content.forEach(function (thing) {
    ...
    Fern.empty_element_tag = function (tag_name) {
        var attributes_and_content = _parse_attributes_and_content.apply(_parse_attributes_and_content, Array.prototype.slice.call(arguments, 1));
        var real_attributes        = attributes_and_content[0];
        var real_content           = attributes_and_content.slice(1);

        return function () {
            var stringified_content_and_metadata = _stringify_content(real_content, Array.prototype.slice.call(arguments, 0));
            var stringified_content              = stringified_content_and_metadata[0];
            var metadata                         = stringified_content_and_metadata.length > 1 ? stringified_content_and_metadata.slice(1) : [];
            var xml = "<" + tag_name + _stringify_attributes(real_attributes, Array.prototype.slice.call(arguments, 0));

            if (stringified_content == "") {
                xml += " />";
            }
            else {
                xml += ">" + stringified_content + "</" + tag_name + ">";
            }

            return [xml].concat(metadata);
        };
    };

    Fern.tag = function (tag_name) {
        var attributes_and_content = _parse_attributes_and_content.apply(_parse_attributes_and_content, Array.prototype.slice.call(arguments, 1));
        var real_attributes        = attributes_and_content[0];
        var real_content           = attributes_and_content.slice(1);
        ...
        return function () {
            ...
            var stringified_content              = stringified_content_and_metadata[0];
            var metadata                         = stringified_content_and_metadata.length > 1 ? stringified_content_and_metadata.slice(1) : [];
            return ["<" + tag_name + _stringify_attributes(real_attributes, Array.prototype.slice.call(arguments, 0)) + ">" + stringified_content + "</" + tag_name + ">"].concat(metadata);
        };
    };
})();
